...
import {Pagination, PaginationResponse} from "../../core/interfaces/pagination.ts";
...
export const fetchPropertiesByCity = async (city: string, pagination : Pagination) : Promise<PaginationResponse<PropertyResponse>> => {
    const response =
        await httpClient.get(`${BASE_URL}/api/v1/properties/city/${city}?currentPage=${pagination.currentPage}&pageSize=${pagination.pageSize}`);
    return response.data;
}